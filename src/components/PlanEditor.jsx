...
/**
 * @param {object} props
 * @param {{ name?: string, description?: string, exercises?: Array<{ exerciseId: string, targetSets: number, targetReps: number, restSeconds?: number, notes?: string }> } | null} props.plan
 * @param {(plan: { name: string, description: string, exercises: Array<{ exerciseId: string, targetSets: number, targetReps: number, restSeconds?: number, notes?: string }> }) => void} props.onSave
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-md border border-gray-200">
      <div className="flex items-center justify-between mb-6">
        ...
        <button type="button" onClick={onCancel} className="p-2 text-gray-400 hover:text-gray-600 rounded-lg hover:bg-gray-100">
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div>
              ...
              return (
                <div key={i} className="flex items-center gap-2 p-3 bg-gray-50 rounded-xl">
                  <div className="flex flex-col gap-1">
                    <button type="button" onClick={() => moveExercise(i, -1)} className="text-gray-300 hover:text-gray-500 text-xs">
                      ▲
                    </button>
                    <button type="button" onClick={() => moveExercise(i, 1)} className="text-gray-300 hover:text-gray-500 text-xs">
                      ▼
                  ...
                  <span
                    className={`w-8 h-8 rounded-lg flex items-center justify-center text-white text-sm shrink-0 ${group?.color || 'bg-gray-400'}`}
                  >
                    {group?.icon}
                  </span>
                  ...
                  <div className="flex items-center gap-1">
                    <input
                      type="number"
                      value={ex.targetSets}
                      onChange={(e) => updateExercise(i, 'targetSets', parseInt(e.target.value, 10) || 0)}
                    ...
                    <input
                      type="number"
                      value={ex.targetReps}
                      onChange={(e) => updateExercise(i, 'targetReps', parseInt(e.target.value, 10) || 0)}
                      className="w-12 text-center text-sm border border-gray-200 rounded-lg py-1 focus:outline-none focus:ring-1 focus:ring-primary-500"
                      min="1"
                    />
                  </div>
                  <button type="button" onClick={() => removeExercise(i)} className="p-1.5 text-gray-300 hover:text-red-500 rounded-lg hover:bg-red-50">
          ...
          {showExercisePicker ? (
            <div className="mt-3 border border-gray-200 rounded-xl p-3 bg-gray-50">
              ...
              <button type="button" onClick={() => setShowExercisePicker(false)} className="mt-2 text-sm text-gray-500 hover:text-gray-700">
                Chiudi
              </button>
            </div>
          ) : (
        ...
        <div className="flex gap-3 pt-4 border-t border-gray-100">
          ...
          <button type="button" onClick={onCancel} className="px-6 py-3 bg-gray-100 text-gray-600 rounded-xl font-medium hover:bg-gray-200 transition-colors">
            Annulla
          </button>
        </div>
      </div>
